...
function itemCell:updateAwardSurfacetimeImg(node,obj,param)
	...
	local size = node:getContentSize()
	surfacetimeImg = self:updateImageView(node,'uires/ui/common/num_bg.png','surfacetime_Img',nil,nil,cc.p(size.width-12,size.height-12))
	local timeType = obj:getTimeType()
	local time = obj:getTime()
	local str = ''
	if tonumber(timeType) == 1 then
...
function itemCell:updateExclusive(node,obj,bgName,isLv,isName,isDesc,showType)
	...
	if not awardBgImg then
		...
		if showType then
			...
			local infoTx = self:updateTTFlabel(equipImg,GlobalApi:getLocalStr('EXCLUSIVE_EQUIP_DESC_'..showType),"font/gamefont1.TTF",25,'info_tx',nil,cc.p(55,40),nil,nil,nil,nil,nil,true)
...
function itemCell:createDescInfo(node,diff,diffSize,maxWidth,obj)
	...
	if desc then
		if not descLineImg then
			descLineImg = self:updateImageView(node,'uires/ui/common/tiao_4.png','desc_line_img')
		...
		if not descTitleTx then
			descTitleTx = self:updateTTFlabel(node,GlobalApi:getLocalStr('EXCLUSIVE_DESC_12'),"font/gamefont1.TTF",24,'desc_title_tx',nil,cc.p(diffSize,-diff),COLOR_TYPE.ORANGE,
...
function itemCell:createExclusiveHeroDesc(node,diff,diffSize,maxWidth,obj)
	...
	if showEffectDesc then
		...
		if not specialLineImg then
			specialLineImg = self:updateImageView(node,'uires/ui/common/tiao_4.png','special_hero_line_img')
		...
		if not specialTitleTx then
			specialTitleTx = self:updateTTFlabel(node,GlobalApi:getLocalStr('EXCLUSIVE_DESC_105'),"font/gamefont1.TTF",24,'special_hero_title_tx',nil,cc.p(diffSize,-diff),COLOR_TYPE.ORANGE,
			nil,nil,nil,cc.p(0,1))
		else
...
function itemCell:createSpecialDesc(node,diff,diffSize,maxWidth,obj)
	...
	if showEffectDesc then
		...
		if not specialLineImg then
			specialLineImg = self:updateImageView(node,'uires/ui/common/tiao_4.png','special_line_img')
			specialLineImg:setScale9Enabled(true)
			specialLineImg:setContentSize(cc.size(maxWidth,specialLineImg:getContentSize().height))
		end
		diff = diff + specialLineImg:getContentSize().height/2
		...
		diff = diff + diffHeight
		if not specialTitleTx then
			specialTitleTx = self:updateTTFlabel(node,GlobalApi:getLocalStr('EXCLUSIVE_DESC_11'),"font/gamefont1.TTF",24,'special_title_tx',nil,cc.p(diffSize,-diff),COLOR_TYPE.ORANGE,
			nil,nil,nil,cc.p(0,1))
		else
			specialTitleTx:setPosition(cc.p(diffSize,-diff))
		end
...
function itemCell:createAttrInfo(node,diff,diffSize,maxWidth,obj)
	...
	if not attrLineImg then
		attrLineImg = self:updateImageView(node,'uires/ui/common/tiao_4.png','attr_line_img')
	...
	if not attrTitleTx then
		attrTitleTx = self:updateTTFlabel(node,GlobalApi:getLocalStr('EXCLUSIVE_DESC_10'),"font/gamefont1.TTF",24,'attr_title_tx',nil,cc.p(diffSize,-diff),COLOR_TYPE.ORANGE,
		nil,nil,nil,cc.p(0,1))
	else
...
function itemCell:createAllAttrInfo(node,diff,diffSize,attrDiff,maxWidth,attrInfo)
	...
	-- diff = diff + diffHeight
	if not attrTitleTx then
		attrTitleTx = self:updateTTFlabel(node,GlobalApi:getLocalStr('EXCLUSIVE_MAIN_TITLE_DESC_1'),"font/gamefont1.TTF",24,'attr_title_tx',
	...
	if not attrBgImg then
		attrBgImg = self:updateImageView(node,'uires/ui/common/tiao_mid_2.png','attr_bg_img',nil,nil,cc.p(0,-diff),cc.p(0,1))
...
function itemCell:createAllSpecialBgImg(node,diff,maxWidth,oldDiff)
	...
	if oldDiff then
		if not specialBgImg then
			specialBgImg = self:updateImageView(node,'uires/ui/common/tiao_mid_2.png','special_attr_bg_img',-1,nil,cc.p(0,-oldDiff),cc.p(0,1))
...
function itemCell:createAllSpecialTitleTx(node,diff,diffSize)
	...
	diff = diff + diffHeight
	if not specialTitleTx then
		specialTitleTx = self:updateTTFlabel(node,GlobalApi:getLocalStr('EXCLUSIVE_MAIN_TITLE_DESC_2'),"font/gamefont1.TTF",24,
...
function itemCell:createAllSpecialHero(node,diff,diffSize,attrDiff,maxWidth,roleObj)
	...
	if name and showEffectDesc then
		diff = diff + diffHeight
		if not specialTitleTx then
			specialTitleTx = self:updateTTFlabel(node,GlobalApi:getLocalStr('EXCLUSIVE_MAIN_TITLE_DESC_3'),"font/gamefont1.TTF",24,
		...
		local oldDiff = diff
		if not attrBgImg then
			attrBgImg = self:updateImageView(node,'uires/ui/common/tiao_mid_2.png','attr_hero_bg_img',nil,nil,cc.p(0,-diff),cc.p(0,1))
			attrBgImg:setScale9Enabled(true)
		else
			attrBgImg:setPosition(cc.p(0,-diff))
		end
