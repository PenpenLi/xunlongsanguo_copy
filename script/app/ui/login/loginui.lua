...
function LoginUI:checkUpdate()
	...
	--]]
	-- 检查小版本是否需要更新
	-- local needSmallUpdate = true
	-- local moveScriptFileStr = cc.UserDefault:getInstance():getStringForKey("move_script_file", "")
	-- if moveScriptFileStr == "1" then
	-- 	local tempManifestPath = cc.FileUtils:getInstance():getWritablePath() .. "update/project.manifest.temp"
	-- 	if cc.FileUtils:getInstance():isFileExist(tempManifestPath) then
	-- 		local content = cc.FileUtils:getInstance():getStringFromFile(tempManifestPath)
	-- 		local fileObj = json.decode(content)
	-- 		if fileObj and fileObj.version == version then
	-- 			needSmallUpdate = false
	-- 		end
	-- 	end
	-- else
	-- 	local manifestPath = cc.FileUtils:getInstance():getWritablePath() .. "update/project.manifest"
	-- 	if cc.FileUtils:getInstance():isFileExist(manifestPath) then
	-- 		local content = cc.FileUtils:getInstance():getStringFromFile(manifestPath)
	-- 		local fileObj = json.decode(content)
	-- 		if fileObj and fileObj.version == version then
	-- 			needSmallUpdate = false
	-- 		end
	-- 	elseif isClientVersionEqualServerVersion then
	-- 		needSmallUpdate = false
	-- 	end
	-- end
	--[[
	...
	if openUpdate or CCApplication:getInstance():getTargetPlatform() ~= kTargetWindows then
		-- local function startUpdate()
			...
			self:createAME()
		--end
		-- if needSmallUpdate then
		-- 	promptmgr:showMessageBox(GlobalApi:getLocalStr("WHETHER_DOWNLOAD_UPDATE"), MESSAGE_BOX_TYPE.MB_OK_CANCEL, function ()
		-- 		startUpdate()
		-- 	end, nil, nil, function ()
		-- 		if Third:Get():isShowQuitGameBySDK() then
		-- 			Third:Get():showQuitGameBySDK()
		-- 		else
		-- 			if Third:Get():isQuitGameBySDK() then
		-- 				Third:Get():quitGameBySDK()
		-- 			else
		-- 				Third:Get():quitGame()
		-- 			end
		-- 		end
