...
function LegionWarRankInfoUI:initcell(index)
    local bgimg = self.rankcelltab[index]:getChildByName("bg_img")
    ...
    icon:loadTexture('uires/ui/legionwar/'..tostring(self.rankconf[index].icon))
    ...
    local lightimg = ccui.ImageView:create("uires/ui/common/light_bg4.png")
    bgimg:setContentSize(cc.size(lightimg:getContentSize().width,lightimg:getContentSize().height))
    if index%2 ==0 then
    ...
    if index <= rankid then
        ...
        if index == rankid then
            ...
            bgimg:setContentSize(cc.size(lightimg:getContentSize().width,lightimg:getContentSize().height))
            bgimg:loadTexture('uires/ui/common/light_bg4.png')
        end
    else
