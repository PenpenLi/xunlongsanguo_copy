...
function JadesealGetAwardUI:init()
    ...
    titlenameimg:loadTexture('uires/ui/jadeseal/'..self.jadesealconf[self.jadesealid].jadesealnameicon)
    ...
    jadesealimg:loadTexture('uires/ui/jadeseal/'..self.jadesealconf[self.jadesealid].jadesealicon)
...
function JadesealGetAwardUI:_onShowUIAniOver()
    ...
    local winSize = cc.Director:getInstance():getWinSize()
    local spineAni = GlobalApi:createLittleLossyAniByName('ui_juanzhou_01')
    spineAni:setPosition(cc.p(self.bgimg:getContentSize().width/2,winSize.height/2))
    spineAni:setLocalZOrder(101)
    spineAni:setScale(1.8)
    self.bgimg:addChild(spineAni)
    spineAni:getAnimation():playWithIndex(0, -1, 0)
