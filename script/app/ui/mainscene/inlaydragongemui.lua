...
function InlayDragonGemUI:init()
            ...
            elseif eventType == ccui.TouchEventType.ended then
                if self.selectIndex ~= i then
                    ...
                    if self.addImgs[i]:isVisible() then
                        self.select_img:loadTexture("uires/ui/treasure/select_bg_2.png")
                    else
                        self.select_img:loadTexture("uires/ui/treasure/select_bg_1.png")
                    end
    ...
    left_btn:addTouchEventListener(function (sender, eventType)
        ...
        elseif eventType == ccui.TouchEventType.ended then
            ...
            if self.currDragonId == 1 then
                self.currDragonId = self.maxDragon
            end
            self:closeDragonBag()
            self:update()
    ...
    self.stars = {}
    for i = 1, 5 do
        local starImg = self.left_bg:getChildByName("star_bg_" .. i)
        self.stars[i] = starImg:getChildByName("star")
    end
...
function InlayDragonGemUI:updateLeft(dragonObj)
    ...
    if self.currDragonId < id then
        level = #treasureConf[self.currDragonId]
    elseif self.currDragonId == id then
    ...
    if self.dragonSpines[self.currDragonId] then
        ...
        dragonSpine:setPosition(cc.p(180, 100))
    ...
    self.name_img:loadTexture("uires/ui/treasure/skill_name_" .. (self.currDragonId - 1) .. ".png")
...
function InlayDragonGemUI:updateRight(dragonObj)
    ...
    for i = 1,5 do
        ...
        if dragonGem then
            ...
            self.quanImgs[i]:loadTexture("uires/ui/treasure/inlay_quan.png")
