...
function InfiniteBattleMain:init()
	...
	bg_img:loadTexture("uires/ui/infinitebattle/bg_"..self.bgIndex..".jpg")
...
function InfiniteBattleMain:createPoint()
    ...
    for i=#conf,1,-1 do
    	...
    	if conf[i].isBranch == 0 then -- 主线
	    	if self.chapterId < infiniteData.chapter_id then
	    		pointBtn:loadTextures('uires/ui/infinitebattle/point_3.png','','')
	    		isCross = true
	    	elseif self.chapterId == infiniteData.chapter_id then
		    	...
		    	elseif i == infiniteData.city_id then
		    		pointBtn:loadTextures('uires/ui/infinitebattle/point_'..infiniteData.progress..'.png','','')
		    		...
		    		pointBtn:loadTextures('uires/ui/infinitebattle/point_-1.png','','')
		    		isNotOpen = true
		    	end
	    	else
	    		pointBtn:loadTextures('uires/ui/infinitebattle/point_-1.png','','')
	    	...
	    	if branchObj then -- 当前这条支线已经开启
	    		...
	    		if pointId then
	    			...
	    			elseif tonumber(pointId) == i then
	    				...
	    				pointBtn:loadTextures('uires/ui/infinitebattle/point_'..progress..'.png','','')
	    				if progress >= 3 then
	    					isCross = true
	    				end
	    			else
...
function InfiniteBattleMain:createSpecialItem()
	...
	for i,v in ipairs(conf) do
	    ...
	    elseif v.type == 'boss' then
	    	specialBtn = ccui.Button:create(v.url,'','')
	        specialBtn:setPosition(cc.p(v.pos[1],v.pos[2]))
	        local txBg = ccui.ImageView:create('uires/ui/legion/building_bg.png')
