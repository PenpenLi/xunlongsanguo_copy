...
function SidebarUI:getSzData(ntype)
    ...
    elseif ntype == 16 or ntype == 'love' then
        tab.icon = 'uires/ui/res/love.png'
...
function SidebarUI:updateSz(i,tab,ntype)
	if type(tab) == 'table' and tab.num then
		...
		if tab.num > 100000000 and not ntype then
			...
			self.szTabs[i].numImg:setPosition(cc.p(140 + ((tab.displayAddImg == true and 0) or 1)*30,18))
			self.szTabs[i].numTx:setPosition(cc.p(bgSize.width - 50 - size.width*0.9 + ((tab.displayAddImg == true and 0) or 1)*30,bgSize.height/2))
		elseif tab.num > 100000 and not ntype then
			...
			self.szTabs[i].numImg:setPosition(cc.p(140 + ((tab.displayAddImg == true and 0) or 1)*30,18))
			self.szTabs[i].numTx:setPosition(cc.p(bgSize.width - 40 + ((tab.displayAddImg == true and 0) or 1)*30,bgSize.height/2))
		end
...
function SidebarUI:update()
	if UserData:getUserObj() then
		...
		if self.playerInfoNode then
			...
			--     local size1 = vipImg:getContentSize()
			--     fightForceTx:setAnchorPoint(cc.p(0,0.5))
			--     fightForceTx:setPosition(cc.p(105,-5))
			...
			if not vipNum then
				...
				vipNum:setPosition(cc.p(size.width/2 + 25, size.height/2 - 18))
...
function SidebarUI:runBottomBtns(callback,isNotAction)
	...
	if not self.isHideBottomBtn then
		self.bottomBgImg:setPosition(pos1)
		self.bottomBgImg:runAction(cc.Sequence:create(cc.MoveTo:create(num * 0.2,pos2),cc.CallFunc:create(function()
		...
		self.bottomBgImg:setPosition(pos2)
		self.bottomBgImg:runAction(cc.Sequence:create(cc.MoveTo:create(num * 0.2,pos1),cc.CallFunc:create(function()
			self.isBottomBtnActionEnd = true
			if callback then
				callback()
			end
...
function SidebarUI:btnsFly(key, callback)
	...
	if key == 'hook' then
		...
		local nameTx = cc.Label:createWithTTF(GlobalApi:getLocalStr('EQUIP_TYPE_'..obj:getType()), "font/gamefont1.TTF", 20)
...
function SidebarUI:updateFrameBtnsNewImgs()
    ...
    local signs1 = {
        ...
        UserData:getUserObj():getSignByType('weapon'),
...
function SidebarUI:setFrameBtnsPosition(b)
	self.vIds = {"hero","bag","treasure","task","legion","country"}
	local extraMask = {}
	for i,v in ipairs(self.vIds) do
		extraMask[i] = GlobalApi:getOpenInfo(v)
	end
...
function SidebarUI:registerBtnHandler()
    ...
    self.hButtons = {
        heroBtn,bagBtn,treasureBtn,taskBtn,legionBtn,countryWarBtn
    }
