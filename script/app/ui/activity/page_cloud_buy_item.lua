...
function CloudBuyItemUI:updateEditBox()
	...
	self.editbox = cc.EditBox:create(cc.size(100, 40), 'uires/ui/common/num_bg4.png')
...
function CloudBuyItemUI:updateTopPanel()
	...
	if not awardBgImg then
		...
		local infoTx = ClassItemCell:updateTTFlabel(signImg,GlobalApi:getLocalStr(str[self.conf1.type]),"font/gamefont1.TTF",16,'info_tx',nil,cc.p(45,45))
	...
	bar:setContentSize(cc.size(578,18))
...
function CloudBuyItemUI:updateRecordCell(node,diff,index,maxWidth,data)
	...
	local lineImg = ClassItemCell:updateImageView(node,'uires/ui/common/tiao_4.png','line_img_'..index,nil,nil,
...
function CloudBuyItemUI:updateMyRecord(node,diff,maxWidth,data)
	...
	local lineImg = ClassItemCell:updateImageView(node,'uires/ui/common/tiao_4.png','line_img',nil,nil,
		cc.p(0,-diff),cc.p(0,0.5))
	lineImg:setScale9Enabled(true)
	lineImg:setContentSize(cc.size(maxWidth,lineImg:getContentSize().height))
...
function CloudBuyItemUI:updateBottomRightPanel()
	...
	if self.page == 1 then
		...
		if self.data1.globalLog and num > 0 then
			...
			ClassItemCell:updateTTFlabel(neiBgImg,'','font/gamefont1.TTF',34,'desc_tx')
		else
			sv1:setVisible(false)
			sv2:setVisible(false)
			ClassItemCell:updateTTFlabel(neiBgImg,GlobalApi:getLocalStr('ACTIVITY_CLOUD_BUY_DESC_27'),'font/gamefont1.TTF',34,'desc_tx',nil,
		...
		if self.data1.log and #self.data1.log > 0 then
			...
			sv2:setVisible(false)
			local size = neiBgImg:getContentSize()
			ClassItemCell:updateTTFlabel(neiBgImg,GlobalApi:getLocalStr('ACTIVITY_CLOUD_BUY_DESC_28'),'font/gamefont1.TTF',34,'desc_tx',nil,
				cc.p(size.width/2,size.height/2),COLOR_TYPE.PALE,COLOROUTLINE_TYPE.PALE,2)
		end
	end
end
