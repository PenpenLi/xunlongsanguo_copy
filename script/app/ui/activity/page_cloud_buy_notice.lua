...
function CloudBuyNoticeUI:updateMyAward(bgImg,i)
	...
	if not awardBgImg then
		...
		local infoTx = ClassItemCell:updateTTFlabel(signImg,GlobalApi:getLocalStr(str[ntype]),"font/gamefont1.TTF",16,'info_tx',nil,cc.p(45,45))
...
function CloudBuyNoticeUI:updateNotice(bgImg,data)
	...
	if not awardBgImg then
		...
		local signImg = ClassItemCell:updateImageView(awardBgImg,'uires/ui/common/corner_blue_'..(data.type or 1)..'.png','sign_img',nil,nil,
			cc.p(size.width + 2,size.height + 2),cc.p(1,1))
		signImg:setScale(0.8)
		local infoTx = ClassItemCell:updateTTFlabel(signImg,GlobalApi:getLocalStr(str[(data.type or 1)]),"font/gamefont1.TTF",16,'info_tx',nil,cc.p(45,45))
		infoTx:setSkewX(45.38)
		infoTx:setSkewY(-45.38)
	end
	local nameTx = awardBgImg:getChildByName('name_tx')
	...
	if not awardBgImg then
		...
		local infoTx = ClassItemCell:updateTTFlabel(awardBgImg,GlobalApi:getLocalStr('ACTIVITY_CLOUD_BUY_DESC_34'),"font/gamefont1.TTF",22,
