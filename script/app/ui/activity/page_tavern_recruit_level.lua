...
function TavernRecruit:init(msg)
    ...
    self.tavern_recruit_level = msg.tavern_recruit_level

    UserData:getUserObj().activity.tavern_recruit_level = self.tavern_recruit_level
...
function TavernRecruit:updateMark()
    if UserData:getUserObj():getSignByType('tavern_recruit_level') then
		ActivityMgr:showMark("tavern_recruit_level", true)
	else
		ActivityMgr:showMark("tavern_recruit_level", false)
	end
end
...
function TavernRecruit:initTop()
    ActivityMgr:showRightTavernRecruitLevelRemainTime()
    ActivityMgr:showLefTavernRecruitCue()   -- 帮助按钮
end
...
function TavernRecruit:refreshCount()
    ...
    tipsCount:setString(self.tavern_recruit_level.frequency or 0)
end
...
function TavernRecruit:refreshSv()
    ...
    local num = self.tavern_recruit_level.num
    local id = num + 1
    local showId = id
    -- 看是否达到次数
    ...
    if id > #self.avTavernRecruitFrequencyConf then
        ...
        if self.tavern_recruit_level.frequency >= self.avTavernRecruitFrequencyConf[id].num then
            judge = 2
        else
            judge = 1
        end
    ...
    for i = 1,num,1 do
        ...
        if roleobj:getCamp() == 1 then
            county:setString(GlobalApi:getLocalStr("ACTIVE_TAVERN_RECRUIT_DES7"))
        elseif roleobj:getCamp() == 2 then
            county:setString(GlobalApi:getLocalStr("ACTIVE_TAVERN_RECRUIT_DES8"))
        elseif roleobj:getCamp() == 3 then
            county:setString(GlobalApi:getLocalStr("ACTIVE_TAVERN_RECRUIT_DES9"))
        else
            county:setString(GlobalApi:getLocalStr("ACTIVE_TAVERN_RECRUIT_DES10"))
        end
                ...
                elseif judge == 2 then
                    ...
                    promptmgr:showMessageBox(
				        ...
				        function()
					        ...
					        MessageMgr:sendPost('get_tavern_recruit_level_generals','activity',json.encode(args),function (jsonObj)
                            print(json.encode(jsonObj))
                                if jsonObj.code == 0 then
                                    ...
                                    self.tavern_recruit_level.num = self.tavern_recruit_level.num + 1
                                    UserData:getUserObj().activity.tavern_recruit_level.num = self.tavern_recruit_level.num
                                    self:refreshSv()
                                    self:updateMark()
                                end
                            end)
...
function TavernRecruit:initRightBottom()
    ...
    local percent = 0
    local hasNum = self.tavern_recruit_level.frequency
    ...
    for i = 1,5 do
        ...
        -- 判断次数是否达到
        local frequency = self.tavern_recruit_level.frequency
        local state = 1
        if frequency >= self.avTavernRecruitConf[i].num then
            local rewards = self.tavern_recruit_level.rewards
            ...
            elseif eventType == ccui.TouchEventType.ended then
                if state ~= 3 then
                    local function callBack()
                        self.tavern_recruit_level.rewards[tostring(i)] = 1
                        UserData:getUserObj().activity.tavern_recruit_level.rewards = self.tavern_recruit_level.rewards
                        self:initRightBottom()
                        self:updateMark()
                    end
                    local activeTavernActiveBoxUI = ActiveTavernActiveBoxUI.new(i,callBack,state,true)
