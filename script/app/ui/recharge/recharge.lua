...
function RechargeUI:updateRechargePanel()
	...
	for i=1,#conf do
		if not self.cells[i] then
			...
			awardImg:loadTexture('uires/ui/recharge/icon_'..i..'.png')
...
function RechargeUI:updatePrivilegePanel()
	...
	if not self.privilegeRts then
	    ...
	    local re3 = xx.RichTextImage:create('uires/ui/res/cash.png')
	...
	if not self.cashRts then
	    ...
	    local re2 = xx.RichTextImage:create('uires/ui/res/cash.png')
...
function RechargeUI:updatePanel()
	...
	boxImg:loadTexture('uires/ui/recharge/box_'..self.page..'.png')
