...
function GuideBattle2UI:initCompleted()
    ...
    self.longdan = GlobalApi:createLittleLossyAniByName("guide_qiangdan")
    self.longdan:setScale(0.6)
    self.longdan:setPosition(cc.p((LEGION_POS[1][2].x + LEGION_POS[2][2].x)/2, LEGION_POS[1][2].y))
    self.bgImg:addChild(self.longdan)
    self.longdan:getAnimation():play("idle", -1, -1)

    if self.armyMap[1] then
        self.armyMap[1]:setVisible(false)
    end
    if self.armyMap[3] then
        self.armyMap[3]:setVisible(false)
    end
...
function GuideBattle2UI:otherSpecialHandle()
    ...
    local label = cc.Label:createWithTTF("", "font/gamefont1.TTF", 38)
    ...
    local talks = {
        {1, cc.pAdd(cc.p(0, 80), LEGION_POS[1][2]), "GUIDE_TEXT_30"},
        {2, cc.pAdd(cc.p(0, 80), LEGION_POS[2][1]), "GUIDE_TEXT_31"},
        {0, monsterConf.pos5},
        {1, cc.pAdd(cc.p(0, 80), LEGION_POS[1][1]), "GUIDE_TEXT_32"},
        {1, cc.pAdd(cc.p(0, 80), LEGION_POS[1][3]), "GUIDE_TEXT_33"}
    }
    local talkIndex = 1
    ...
    local function showTalk()
        if talkIndex <= totalIndex then
            if talks[talkIndex][1] == 0 then -- 显示boss介绍界面
                ...
                dialog:runAction(cc.Sequence:create(cc.DelayTime:create(1), cc.CallFunc:create(function ()
...
function GuideBattle2UI:updatePlayerSkill(guid)
    ...
    if not self.playerSkillStatusArr[1][1].disable and self.playerSkillStatusArr[1][1].time == 0 then
        ...
        label:setFontName("font/gamefont1.TTF")
        ...
        dialogNode:setPosition(cc.p(-160, 150))
        label:setString(guidetextConf["GUIDE_TIPS_43"].text)
        ...
        dialogNode:runAction(cc.Sequence:create(cc.DelayTime:create(3/TIMESCALE), cc.CallFunc:create(function ()
        ...
        end), cc.DelayTime:create(3/TIMESCALE), cc.CallFunc:create(function ()
            ...
            local handEndPos = cc.p(size.width/2, size.height/2 + 200)
